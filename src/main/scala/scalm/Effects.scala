...
object Sub {
  ...
  def ofTotalObservable[Msg](
      id: String,
      observable: Observable[Nothing, Msg]
  ): Sub[Msg] =
      ...
      { observer =>
        val handle =
          dom.window.setTimeout(
            Functions.fun0(() => observer.onNext(msg)),
      ...
      { observer =>
        val handle =
          dom.window.setInterval(
            Functions.fun0(() => observer.onNext(new js.Date())),
  ...
  def fromEvent[A, B](name: String, target: EventTarget)(
      extract: A => Option[B]
  ): Sub[B] =
...
sealed trait Task[+Err, +Success] {
  ...
  def product[Success2, Err2 >: Err](
      that: Task[Err2, Success2]
  ): Task[Err2, (Success, Success2)] = Task.Multiplied(this, that)

  /** Sequentially applies this task and then another task given by the function `f` */
  def flatMap[Success2, Err2 >: Err](
      f: Success => Task[Err2, Success2]
  ): Task[Err2, Success2] = Task.FlatMapped(this, f)
  ...
  def perform[Err2 >: Err](implicit ev: Err2 =:= Nothing): Cmd[Success] =
    Cmd.RunTask[Err, Success, Success](this, _.right.get)
...
object Task {
  ...
  case class RunObservable[Err, Success](observable: Observable[Err, Success])
      extends Task[Err, Success]
  case class Recovered[Err, Success](
      task: Task[Err, Success],
      f: Err => Task[Err, Success]
  ) extends Task[Err, Success]
  case class Mapped[Err, Success, Success2](
      task: Task[Err, Success],
      f: Success => Success2
  ) extends Task[Err, Success2]
  case class Multiplied[Err, Success, Success2](
      task1: Task[Err, Success],
      task2: Task[Err, Success2]
  ) extends Task[Err, (Success, Success2)]
  case class FlatMapped[Err, Success, Success2](
      task: Task[Err, Success],
      f: Success => Task[Err, Success2]
  ) extends Task[Err, Success2]
...
object Cmd {
  case class RunTask[Err, Success, Msg](
      task: Task[Err, Success],
      f: Either[Err, Success] => Msg
  ) extends Cmd[Msg] {
    def map[OtherMsg](g: Msg => OtherMsg): Cmd[OtherMsg] =
      RunTask(task, f andThen g)
  }
